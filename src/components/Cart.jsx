...
const Cart = () => {
  ...
  const [totalPrice, setTotalPrice] = useState(0);
  const [cartImage, setCartImage] = useState([]);
  const [cartSong, setCartSong] = useState([]);
  const [showModal, setShowModal] = useState(false);
    ...
    const newCartItems = cartItems.map((item) => {
      if (item.id === itemId) {
        if (item.quantity < item.stockQuantity) {
          ...
          alert("Cannot add more than available song");
        }
      }
      return item;
    });
  ...
  const handleCheckout = async () => {
    try {
      for (const item of cartItems) {
        ...
        const updatedProductData = { ...rest, stockQuantity: updatedStockQuantity };
        console.log("updated song data", updatedProductData)
        ...
        await axios
          ...
          .then((response) => {
            console.log("song updated successfully:", (cartProduct));
          })
          .catch((error) => {
  ...
  return (
    <div className="cart-container">
      <div className="shopping-cart">
        <div className="title">Favourite Songs</div>
        {cartItems.length === 0 ? (
          <div className="empty" style={{ textAlign: "left", padding: "2rem" }}>
            <h4>Your song list is empty</h4>
          </div>
        ) : (
          <>
            {cartItems.map((item) => (
